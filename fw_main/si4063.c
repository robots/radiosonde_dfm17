...
const struct gpio_init_table_t si4063_gpio[] = {
	{ // SDN 
		.gpio = GPIOC,
		.pin = GPIO_Pin_3,
...
static int si4063_wait_for_cts()
{
    uint16_t timeout = 0xFFFF;
		uint8_t cmd = SI4063_COMMAND_READ_CMD_BUFF;
    uint8_t response;
...
static void si4063_send_command(uint8_t command, uint8_t length, uint8_t *data)
{
    si4063_wait_for_cts();
		...
		spi_lock(0);
		spi_cs_on();
		spi_send(&command, NULL, 1);
		spi_send(data, NULL, length);
		spi_cs_off();
		spi_unlock();
}
...
{
		gpio_set(&si4063_gpio[GPIO_SDN], active ? GPIO_SET : GPIO_RESET);
}
...
{
		gpio_set(&si4063_gpio[GPIO_GPIO3], high? GPIO_SET : GPIO_RESET);
}
